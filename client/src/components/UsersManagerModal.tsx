...
import { X, Plus, Trash2, Shield, User as UserIcon, Check } from "lucide-react";
...
export function UsersManagerModal({ open, onClose }: Props) {
  ...
  const [form, setForm] = useState({ username: "", name: "", password: "", role: "user" as "admin" | "user" });
              ...
              users.map((u) => (
                <div
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border border-border ${
                    u.active ? "bg-card" : "bg-muted/30 opacity-60"
                  }`}
                >
